...
public abstract class BaseSession {
    ...
    /**
     * 
     * @Description : Cancel Session & play TTS
     * @Author : xiaodong.he
     * @ModifyDate : 2016-2-1
     */
    public void cancelSession() {
        ...
        cancelSession(SessionPreference.PARAM_TYPE_PLAY_TTS, "");
    }
     ...
     * @param reason: {@link SessionPreference#PARAM_REASON_*}
     */
    public void cancelSession(String type, String reason) {
        // mSessionManagerHandler.sendEmptyMessage(SessionPreference.MESSAGE_CANCEL_TALK);
        addAnswerViewText(mContext.getString(R.string.operation_cancel));
        ...
        data.putString(SessionPreference.KEY_TYPE, type);
        data.putString(SessionPreference.KEY_REASON, reason);
        msg.setData(data);
    ...
    protected void addAnswerView(View view, boolean fullScroll) {
        ...
        addAnswerView(view, fullScroll, Gravity.CENTER_VERTICAL);
    }
     ...
     * @param gravity
     */
    protected void addAnswerView(View view, boolean fullScroll, int gravity) {
        if (view == null) {
            return;
        }
        Logger.d(TAG, "!--->addAnswerView--fullScroll = " + fullScroll + "; gravity = " + gravity);
     ...
     */
    protected void addSessionViewContainsListView(View view, int gravity) {
    ...
    protected void addPushView(View view) {
        if (view == null) {
            return;
        }
        // SessionContainer.addViewNow(view.hashCode());
        Message msg = new Message();
        msg.what = SessionPreference.MESSAGE_ADD_PUSH_VIEW;
        msg.obj = view;
        mSessionManagerHandler.sendMessage(msg);
    }
}
