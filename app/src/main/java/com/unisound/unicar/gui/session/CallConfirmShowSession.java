...
public class CallConfirmShowSession extends ContactSelectBaseSession {
    ...
    private boolean isNeedToRestartCountDown = false;
    ...
    private void executeCall(String number) {
        ...
        if (mContext != null) {
            try {
                RomCustomerProcessing.sendMessageToOtherCall(mContext, number);
    ...
    public void needToCancelCountDownTime() {

        Logger.i(TAG, "needToCancelCountDownTime :" + isTimeOut);
    ...
    public void onTTSEnd() {
        ...
        super.onTTSEnd();
        if (isNeedToRestartCountDown) {
            isNeedToRestartCountDown = false;
            if (mCallContentView != null) {
                long time = mCallContentView.getCountdownTime();
                mCallContentView.startCountDownTimer(time);
            }
            return;
        }
