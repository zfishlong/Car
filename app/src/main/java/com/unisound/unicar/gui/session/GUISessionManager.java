 ...
 * 
 * @FileName : GUISessionManager.java
...
public class GUISessionManager {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case SessionPreference.MESSAGE_SESSION_CANCEL:
                    // XD 20160201 modify for cancel session
                    Bundle cancelData = msg.getData();
                    String type = "";
                    String reason = "";
                    if (null != cancelData) {
                        type =
                                cancelData.getString(SessionPreference.KEY_TYPE,
                                        SessionPreference.PARAM_TYPE_PLAY_TTS);
                        reason = cancelData.getString(SessionPreference.KEY_REASON, "");
                    }
                    Logger.d(TAG, "SESSION_CANCEL---type: " + type + "; reason: " + reason);
                    if (SessionPreference.PARAM_TYPE_PLAY_TTS.equals(type)
                            || TextUtils.isEmpty(type)) {
                        // GUI Clicked Cancel button
                        mWindowService.onCancelTalk();
                    } else if (SessionPreference.PARAM_TYPE_NOT_PLAY_TTS.equals(type)) {
                        mWindowService.onCancelTalk(false, reason);
                    }
                    break;
                ...
                case SessionPreference.MESSAGE_ADD_ANSWER_VIEW:
                    ...
                    int gravity = msg.arg2;
                    Logger.d(TAG, "!--->--MESSAGE_ADD_ANSWER_VIEW---arg1 = " + msg.arg1
                            + ";fullScroll = " + fullScroll + "; gravity" + gravity);
                    mSessionViewContainer.removeAllSessionViews();
                    if (gravity != 0) {
                        mSessionViewContainer.addSessionView(view, fullScroll, gravity);
                    } else {
                    ...
                    break;
                case SessionPreference.MESSAGE_ADD_PUSH_VIEW:
                    PushView viewPush = (PushView) msg.obj;
                    if (mWindowService != null) {
                        mWindowService.addPushView(viewPush);
                    }
                    break;
                case SessionPreference.MESSAGE_ADD_SESSION_LIST_VIEW:
                    // xd added 20150716
                    View listView = (View) msg.obj;
                    int gravity1 = msg.arg1;
                    Logger.d(TAG, "!--->--ADD_SESSION_LIST_VIEW--gravity:" + gravity1);
                    mSessionViewContainer.removeAllSessionViews();
                    mSessionViewContainer.addSessionListView(listView, gravity1);
                    break;
                // TODO 添加问句文字（用户说的话）
                ...
                case SessionPreference.MESSAGE_SHOW_EDIT_LOCATION_POP:
                    ...
                    String toPoi = (String) msg.obj;
                    mWindowService.showEditLocationPopWindow(mWindowService, toPoi);
                    break;
    ...
    public GUISessionManager(WindowService windowService, SessionContainer sessionViewContainer,
        ...
        HandlerStore.getInstance().put("SessionManagerHandler", mSessionManagerHandler);
    ...
    private void showLoadingView(boolean isShow, boolean isFailed, String titleText, String tipText) {
        ...
        if (isShow) {
            ...
            if (WelcomeActivity.isInWelcomeActivity()) {
                return;
            }
    ...
    private void dismissWelcomeActivity() {
        ...
        Logger.d(TAG, "dismissWelcomeActivity--begin--send DIMISS_WELCOME_ACTIVITY delay 2s");
        // XD added 2016-1-12 for Fix bug when kill GUI process on some special ROM device, the
        // WelcomeActivity do not dismiss
        mSessionManagerHandler.sendEmptyMessageDelayed(
                SessionPreference.MESSAGE_DIMISS_WELCOME_ACTIVITY, 2000);
    }
    ...
    private void sendDismissWelcomeActivityBroadcast() {
        ...
        // mWindowService.sendDismissWelcomeActivityBroadcast();
    ...
    private void addSessionViewWithoutScrolling(View view) {
        ...
        msg.obj = view;
        msg.arg1 = Gravity.CENTER_VERTICAL;
     ...
     * @param protocal
     */
    public void onUiProtocal(String eventName, String protocal) {
        ...
        public void onCallStateChanged(int state, String incomingNumber) {
            ...
            switch (state) {
                case TelephonyManager.CALL_STATE_RINGING:
                    ...
                    mWindowService.setPhoneStatus(TelephonyManager.CALL_STATE_RINGING);
                    if (mCurrentSession != null) {
                ...
                case TelephonyManager.CALL_STATE_OFFHOOK:
                    ...
                    mWindowService.setPhoneStatus(TelephonyManager.CALL_STATE_OFFHOOK);
                ...
                case TelephonyManager.CALL_STATE_IDLE:
                    Logger.d(TAG, "!--->--TelephonyManager.CALL_STATE_IDLE--mPhoneState = "
                            + mPhoneState);
                    mWindowService.setPhoneStatus(TelephonyManager.CALL_STATE_IDLE);
     ...
     * @date 2015-11-23
     */
    public void releaseCurrentSessionOnly() {
    ...
    public void onTalkRecordingStop() {
        Logger.d(TAG, "!--->onTalkRecordingStop----ASRResult show: "
                + (mASRResultSessionView != null));
        mMicrophoneControl.onProcess();
        // mMicrophoneControl.setEnabled(true);

        // XD mark: why mMicrophoneControl.setOnClickListener(mMicrophoneClickListener); here?
        // setEnableMicrophoneControl(true); //XD delete 2015-12-07 for UI3
        ...
        // XD modify 20160318
        if (null != mASRResultSessionView) {
            mASRResultSessionView = null;
        } else {
            showMicStatusView(statusText);
        }
    }
    ...
    private void createSession(String protocal) {
        ...
        if (obj != null) {
            data = JsonTool.getJSONObject(obj, SessionPreference.KEY_DATA);
            ...
            // TODO 根据type进行实例创建
            if (mCurrentSession != null && mCurrentSession instanceof CallConfirmShowSession
                    && isErrorSchedule()) {
                ((CallConfirmShowSession) mCurrentSession).restartCountDownTime();
                return;
            }
            ...
            if (SessionPreference.VALUE_SCHEDULE_TYPE_UNSUPPORT.equals(mType)) {
            ...
            } else if (SessionPreference.EVENT_NAME_INCOMING_PUSH_MESSAGE_SHOW.equals(mType)) {
                Logger.d(TAG, "!--->----VALUE_SCHEDULE_TYPE_PUSH_MESSAGE------");
                // 收到来通知消息，先显示窗体
                try {

                    JSONObject dataObject = JsonTool.getJSONObject(obj, SessionPreference.KEY_DATA);
                    String pushModelJson =
                            JsonTool.getJsonValue(dataObject, SessionPreference.KEY_CONTENT);
                    PushModle pushModle = new Gson().fromJson(pushModelJson, PushModle.class);
                    if (pushModle.getPushBaseInfo().getPushType() != PushType.SubscribeMessage
                            && pushModle.getPushBaseInfo().getPushType() != PushType.BackSettingMessage) {
                        mWindowService.show();
                        mMicrophoneControl.setVisibility(View.GONE);
                        base =
                                new ReceivePushMessageSession(mWindowService,
                                        mSessionManagerHandler);
                    }
                } catch (Exception e) {
            ...
            } else if (SessionPreference.VALUE_SCHEDULE_TYPE_INCOMING_PUSH_MESSAGE_OPERATOR
                    .equals(mType)) {
                mWindowService.dismiss();
                base =
                        new ReceivePushMessageOperatorSession(mWindowService,
                                mSessionManagerHandler);
               ...
               */
                ...
                if (SessionPreference.DOMAIN_ROUTE.equals(originType)) {
                    ...
                    isWaitingSessionCanceled = false;
                    base = new LocalsearchWaitingSession(mWindowService, mSessionManagerHandler);
                } else {
            ...
            // TODO add by ch Traffic
                ...
                // XD 2015-11-30 modify for fix bug-4897
                // XD 2016-01-28 modify
                // base = new MainMapLocationSession(mWindowService, mSessionManagerHandler);
                base = new MainInputPoiSession(mWindowService, mSessionManagerHandler);
            ...
            } else if (SessionPreference.VALUE_SCHEDULE_TYPE_LOCATE_CURRENT_POI.equals(mType)) {
                base = PoiChatShowSession.getInstance(mWindowService, mSessionManagerHandler);
            ...
            } else if (SessionPreference.VALUE_SCHEDULE_TYPE_NAVI_CONTROL.equals(mType)) {
                Logger.d(TAG, "SCHEDULE_TYPE_NAVI_CONTROL");
                base = new NaviControlSession(mWindowService, mSessionManagerHandler);
            } else if (SessionPreference.VALUE_SCHEDULE_TYPE_INPUT_WECHAT_CONTACTS.equals(mType)
                    || SessionPreference.VALUE_SCHEDULE_TYPE_INPUT_WECHAT_CONTENT.equals(mType)
                    || SessionPreference.VALUE_SCHEDULE_TYPE_NO_WECHAT_CONTACTS.equals(mType)
                    || SessionPreference.VALUE_SCHEDULE_TYPE_MORE_WECHAT_CONTACTS.equals(mType)) {
                Logger.d(TAG, "schedule type = " + mType);
                if (mWindowService != null) {
                    // mWindowService.playTTs("请先登录微信！",
                    // RomCustomerProcessing.TTS_END_WAKEUP_START);
                    mWindowService.startWechatActivity();
                }
                base = new WechatSession(mWindowService, mSessionManagerHandler);
            }
        }
    ...
    private void showAsrResultTextOnSessionView(String result, boolean isPartial) {
        ...
        tvAsrStatus.setText(result);
        int color =
                isPartial ? mWindowService.getResources().getColor(
                        R.color.color_main_auto_help_text) : mWindowService.getResources()
                        .getColor(R.color.color_asr_result_text);
        tvAsrStatus.setTextColor(color);
    }
    ...
    private boolean isErrorSchedule() {
        if (SessionPreference.VALUE_TYPE_ERROR_SHOW.equals(mType)
                || SessionPreference.VALUE_SCHEDULE_TYPE_UNKNOWN_ERROR.equals(mType)
                || SessionPreference.VALUE_SCHEDULE_TYPE_SCHEDULE_ERROR.equals(mType)
                || SessionPreference.VALUE_SCHEDULE_TYPE_UNSUPPORT.equals(mType)
                || SessionPreference.VALUE_SCHEDULE_TYPE_ONLINE_ERROR.equals(mType)
                || SessionPreference.VALUE_SCHEDULE_TYPE_UNKNOWN_ERROR.equals(mType)) {
            return true;
        }
        return false;
    ...
    public void isNeedToCancelDownTime() {

        Logger.d(TAG, "isNeedToCancelDownTime"
                + (mCurrentSession != null && mCurrentSession instanceof CallConfirmShowSession));
        if (mCurrentSession != null && mCurrentSession instanceof CallConfirmShowSession) {
            ((CallConfirmShowSession) mCurrentSession).needToCancelCountDownTime();
        }
    }
